...
- **Note 2:** I installed Anaconda for python v 3.7 (yours might be different when you install)

- **Then:** In NEW SHELL/TERMINAL, enter 'which python' or 'which anaconda' ('where' instead of 'which' on PCs) to get path to updated Python version. For example, mine returns: `/opt/anaconda3/bin/python`

#### a. Tell R which Python version to use 
```{r}
use_python("/opt/anaconda3/bin/python")
py_config()
```

#### b. Install Python modules with pip

Enter in RStudio terminal for installing new modules to the right place (e.g. `dplython`): 

`/opt/anaconda3/bin/pip install dplython`

(...or whatever your path is to python so it gets sent to the right place), for whatever module (e.g pandas, numpy, etc.) you want to install.
